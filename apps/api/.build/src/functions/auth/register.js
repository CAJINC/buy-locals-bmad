...
app.post('/auth/register', registrationRateLimit, validateBody(registerSchema), async (req, res, next) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO users (id, email, password_hash, role, profile, is_email_verified, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
       RETURNING id, email, role, profile, is_email_verified, created_at, updated_at`, [dbUserId, userData.email, '', userData.role || 'consumer', JSON.stringify(profile), false]);
