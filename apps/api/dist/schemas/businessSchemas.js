...
const hoursSchema = Joi.object().pattern(Joi.string().valid('monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'), Joi.object({
    open: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).optional().messages({
        'string.pattern.base': 'Opening time must be in HH:MM format (e.g., 09:00, 17:30)',
    }),
    close: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).optional().messages({
        'string.pattern.base': 'Closing time must be in HH:MM format (e.g., 09:00, 17:30)',
    }),
    closed: Joi.boolean().optional(),
}).or('open', 'closed').and('open', 'close').messages({
    'object.missing': 'Each day must specify either opening/closing times or be marked as closed',
...
const contactSchema = Joi.object({
    phone: Joi.string().pattern(/^\+?1?[-.\s]?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}$/).optional().messages({
        'string.pattern.base': 'Please provide a valid US phone number (e.g., (555) 123-4567, 555-123-4567)',
    }),
    email: Joi.string().email({ tlds: { allow: false } }).max(254).optional().messages({
        'string.email': 'Please provide a valid email address',
        'string.max': 'Email address cannot exceed 254 characters',
    }),
    website: Joi.string().uri({ scheme: ['http', 'https'] }).max(2048).optional().custom((value, helpers) => {
...
export const createBusinessSchema = Joi.object({
    ...
    categories: Joi.array().items(Joi.string().valid(...getAllCategories()).messages({
        'any.only': 'Category must be one of the predefined business categories',
    })).min(1).max(3).unique().required().messages({
...
export const updateBusinessSchema = Joi.object({
    ...
    categories: Joi.array().items(Joi.string().valid(...getAllCategories()).messages({
        'any.only': 'Category must be one of the predefined business categories',
    })).min(1).max(3).unique().optional().messages({
        'array.min': 'At least one category is required',
        'array.max': 'Maximum 3 categories allowed',
        'array.unique': 'Categories must be unique',
    }),
...
//# sourceMappingURL=businessSchemas.js.map