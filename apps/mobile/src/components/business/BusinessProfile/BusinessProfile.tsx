...
export const BusinessProfile: React.FC<BusinessProfileProps> = React.memo(
  ({
    ...
    const logoImage = useMemo(
      () => business?.media?.find(m => m.type === 'logo'),
      [business?.media]
    );

    const photos = useMemo(
      () => business?.media?.filter(m => m.type === 'photo') || [],
      [business?.media]
    );
      ...
      >
        <VStack space={responsive.getSpacing(24)} p={responsive.getSpacing(16)}>
          ...
          <VStack space={3}>
            ...
            <BusinessHoursDisplay hours={business.hours} showCurrentStatus={true} />
          </VStack>
                ...
                <BusinessPhotoGallery
                  ...
                  onImagePress={(imageUrl, index) => {
                    const accessibilityLabel =
                      AccessibilityHelper.getPhotoGalleryAccessibilityLabel(photos, index);
